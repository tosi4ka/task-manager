...
- **Backend:** Go + Gin, PostgreSQL + sqlx, JWT (access + refresh), slog, Redis, Docker
- **Планируется:** Swagger, Rate limiting, WebSockets
...
backend/
  ...
  internal/
    ...
    server/middleware.go           — JWT middleware
    ...
    task/
      ...
      repository.go                — SQL запросы (в разработке)
      service.go                   — бизнес-логика
      service_test.go
...
- Tasks CRUD: handler, service, model — покрыты TDD (service_test.go)
- Миграции: users + tasks таблицы
- Роутер с группировкой `/api/v1`

## В процессе

- `task/repository.go` — SQL реализация (CreateTask частично, UpdateTask — заглушка)
- Нужно: GetByID, DeleteTask, ListTasks (с фильтрами по статусу/пользователю)
